...
void convolveX(unsigned char *inputPixels, int width, int height, unsigned char *&outputPixels) {
    // Sobel filter for x-direction
    int sobelFilter[3][3] = {{1, 0, -1}, {2, 0, -2}, {1, 0, -1}};
    ...
    for (int y = 1; y < height - 1; y++) {
        for (int x = 1; x < width - 1; x++) {
            ...
            for (int i = -1; i <= 1; i++) {
                for (int j = -1; j <= 1; j++) {
                    sum += sobelFilter[i + 1][j + 1] * inputPixels[(y + i) * width + (x + j)];
                }
            }
            // Ensure the result is within the valid range [0, 255]
            outputPixels[y * width + x] = (unsigned char)(sum > 255 ? 255 : (sum < 0 ? 0 : sum));
        }
    }
}
...
void findOptSeam(unsigned char *energy, int rows, int columns, unsigned char *optSeamMask) {
    ...
    //     for (int j = 0; j < columns; ++j) {
    //         printf("%3d ", optSeamMask[i * columns + j]);
    //     }
    //     printf("\n");
    // }
    ...
    for (int i = 0; i < rows; ++i) {
        ...
        for (int j = 0; j < columns; ++j) {
            if (seamMask[i * columns + j] == 0) {
                energyMatrix[i * (columns) + newColIndex] = energyMatrix[i * columns + j];
                ++newColIndex;
            }
        }
    }
...
void seamCarvingImage(uchar3 *energyMatrix, uchar3 *newEnergyMatrix, int rows, int columns, int n)
{
  for(int k=0; k<n; k++)
  {
    ...
    optSeamMaskMatrix = (unsigned char *)malloc(rows * columns * sizeof(unsigned char));
...
int main(int argc, char ** argv)
{
  ...
  outPixels = (uchar3 *)malloc((rows) * (columns-k) * sizeof(uchar3));
  seamCarvingImage(inPixels, outPixels, rows, columns, k);
  printf("DONE1");
  char * outFileNameBase = strtok(argv[2], "."); // Get rid of extension
  writePnm(outPixels, rows, columns-k, concatStr(outFileNameBase, "_host.pnm"));
  printf("DONE");
